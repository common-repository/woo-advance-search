=== Advance Search for WooCommerce ===
Plugin Name: Advance Search for WooCommerce
Plugin URI: https://www.thedotstore.com/
Author: theDotstore
Author URI: https://www.thedotstore.com/
Contributors: dots
Stable tag: 2.0.3
Tags: search,advance search,woocommerce advance search,woocommerce search,woocommerce product search,woocommerce product category search,woocommerce product tag search
Requires at least: 4.0
Tested up to: 5.5.3
Donate link: 
Copyright: (c) 2015-2019 Thedotstore all rights reserved (support@thedotstore.com)
License: GPLv3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html

Advance Search for WooCommerce plugin allows you to add an advanced search option for WooCommerce Products.

== Description ==
Advance Search for WooCommerce plugin allows you to add an advanced search option for WooCommerce Products.

With this option you can search products by product tag and category.

Yu can apply filter searcher like Title, order by date, price category and search order by ascending, Descending.

You can customize search as per your requirement like enable and disable product category and tag.

You can view searcher option by preview option. you can integrated searcher option in your site using a short-code on a page, as the widget in a sidebar or as template tag in a template.

Plugin Demo : <a href ="http://pluginsdemo.thedotstore.com/woo-advance-search/" target="_blank">View Demo</a>

== 💁 Key features for the Advance Search for WooCommerce ==

🔑 Advance search across all your WooCommerce products
🔑 Advance Search by Category
🔑 Advance Search by Product tag
🔑 Filter by Title, Date, Price
🔑 Filter by Ascending and Descending
🔑 Enable or disable searches by product category and tag.
🔑 you can preview of advanced searcher option as per your setting.
🔑 you can apply custom CSS for advanced searcher option.
🔑 you can search product on live ajax search with suggestions.
🔑 Shortcode - Use shortcode to place search option anywhere you want

== 💁 Contact Us ==

<ul>
<li>Free plugin: Need Technical Help? - <a href ="https://wordpress.org/support/plugin/woo-advance-search/" target="_blank">Click here</a></li>
<li>Pro Plugin:  PRE-SALE Questions - <a href ="https://www.thedotstore.com/support/" target="_blank">Click here</a></li>
</ul>

== EXPLORE OUR OTHER PLUGINS TO SUPERCHARGE YOUR WEBSITE: ==

== 👉 Premium WooCommerce Plugins list ==

<ol>
<li><a href="https://wordpress.org/plugins/wpbricks/" target="_blank" rel="noreferrer noopener">WPBricks Manager – Readymade Custom Gutenberg Blocks</a></li>
<li><a href="https://www.thedotstore.com/flat-rate-shipping-plugin-for-woocommerce" target="_blank" rel="noreferrer noopener">Flat-rate shipping plugins for WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-extra-fees-plugin" target="_blank" rel="noreferrer noopener">WooCommerce Extra Fees Plugin</a></li>
<li><a href="https://www.thedotstore.com/hide-shipping-method-for-woocommerce" target="_blank" rel="noreferrer noopener">Hide Shipping Method For WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-product-attachment" target="_blank" rel="noreferrer noopener">Product Attachment For WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-blocker-lite-prevent-fake-orders-blacklist-fraud-customers/" rel="noreferrer noopener" target="_blank">Blocker – Prevent Fake Orders And Blacklist Fraud Customers</a></li>
<li><a href="http://www.thedotstore.com/woocommerce-enhanced-ecommerce-analytics-integration-with-conversion-tracking" rel="noreferrer noopener" target="_blank">Enhanced Ecommerce Google Analytics for WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-category-banner-management/" rel="noreferrer noopener" target="_blank">Category Banner Management for Woocommerce</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-product-finder" target="_blank" rel="noreferrer noopener">Product Finder For WooCommerce</a></li>
<li><a href="https://www.thedotstore.com/mass-pages-posts-creator/" target="_blank" rel="noreferrer noopener">Mass Pages Posts Creator For WordPress</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-conditional-discount-rules-for-checkout" target="_blank" rel="noreferrer noopener">Conditional Discount Rules For WooCommerce Checkout</a></li>
<li><a href="https://www.thedotstore.com/woocommerce-checkout-for-digital-goods/" target="_blank" rel="noreferrer noopener">WooCommerce Checkout For Digital Goods</a></li>
<li><a href="https://www.thedotstore.com/advance-menu-manager-wordpress/" target="_blank" rel="noreferrer noopener">Advance Menu Manager For WordPress</a></li>
<li><a href="https://www.thedotstore.com/page-visit-counter/" target="_blank" rel="noreferrer noopener">Page Visit Counter For WordPress</a></li>
</ol>

= 👉  Some of our other free WooCommerce plugins: =
<ol>
<li><a href="https://wordpress.org/plugins/woo-advance-search/" target="_blank" rel="noreferrer noopener">Advance Search for WooCommerce</a></li>
<li><a href="https://wordpress.org/plugins/woo-order-status-per-product/" target="_blank" rel="noreferrer noopener">Custom Order Status Per Product for WooCommerce</a></li>
<li><a href="https://wordpress.org/plugins/multipurpose-block/" target="_blank" rel="noreferrer noopener">Multipurpose Gutenberg Block</a></li>
<li><a href="https://wordpress.org/plugins/convert-classic-editor-to-blocks/" target="_blank" rel="noreferrer noopener">Convert Classic Editor to Gutenberg Blocks</a></li>
<li><a href="https://wordpress.org/plugins/highlight-blocks/" target="_blank" rel="noreferrer noopener">Highlight Gutenberg Blocks</a></li>
<li><a href="https://wordpress.org/plugins/woo-quick-cart-for-multiple-variations/" target="_blank" rel="noreferrer noopener">Quick Bulk Variations Checkout for WooCommerce</a></li>
<li><a href="https://wordpress.org/plugins/woo-quick-reports/" target="_blank" rel="noreferrer noopener">Quick Reports for Woocommerce</a></li>
<li><a href="https://wordpress.org/plugins/woo-shipping-display-mode/" target="_blank" rel="noreferrer noopener">Shipping Method Display Style for WooCommerce</a></li>
<li><a href="https://wordpress.org/plugins/template-manager/" target="_blank" rel="noreferrer noopener">Template Manager for Gutenberg Block</a></li>
<li><a href="https://wordpress.org/plugins/wp-custom-field-for-gutenberg-editor/" target="_blank" rel="noreferrer noopener">WP Custom field for Gutenberg Editor</a></li></ol>


== Installation ==

* Download the plugin
* Upload the folder "woo-advance-search" to wp-content/plugins (or upload a zip through the Wordpress admin)
* Activate and enjoy!
* You can access the setting page from here WooCommerce -> Settings -> General.
* Enable to change Woo Advance Search settings form here WooCommerce -> Settings -> General.

== Frequently Asked Questions ==

It is compatible all WordPress version.

== Screenshots ==

1. Example of  Woo Advance Search Ovrview 
2. Example of  Woo Advance Search settings.
3. Example of  Woo Advance Search settings.
4. Example of  Woo Advance Search Preview.
5. Example of  Woo Advance Search Shortcode.
6. Example of  Woo Advance Search Ajax results Front-End.
7 Example of  Woo Advance Search result


== Upgrade Notice ==

Automatic updates should work great for you.  As always, though, we recommend backing up your site prior to making any updates just to be sure nothing goes wrong.

== Changelog ==
= 2.0.3 - 07-01-2021 =
* Minor bug fixed

= 2.0.2 - 25-11-2020 =
* Minor bug fixed

= 2.0.1 - 12-11-2020 =
* Compatible with WooCommerce 4.7.x
* Compatible with WordPress 5.5.3

= 2.0 - 17-08-2020 =
* Compatible with WooCommerce 4.3.x
* Compatible with WordPress 5.5.x

= 1.9 - 26-05-2020 =
* Compatible with WooCommerce 4.1.x
* Compatible with WordPress 5.4.x

= 1.8 - 21.11.2019 =
* Compatible with Freemius
* VIP minimum 
* On search Live Ajax data

= 1.7 - 19.11.2019 =
* Compatible with WordPress 5.3.x
* Compatible with WooCommerce 3.8.x
* shortcode issue fixed
* Create new file for admin reusable function file as (woo-advance-search-admin-functions.php) and Configure this file in class-woo-advance-search.php

= 1.6 - 28.05.2019 =
* Compatible with WordPress Version 5.2.x and WooCommerce version 3.6.x

= 1.5 - 12.04.2019 =
* Category and Tags name not searching if its contains space fixed.

= 1.4 - 10.04.2019 =
* Product search by category or condition to and condition login apply.

= 1.3 - 26.03.2019 =
* Minor bug fixing

= 1.2 - 13.03.2019 =
* Minor bug fixing
* Compatible with WordPress 5.1.1 and WooCommerce 3.5.6

= 1.1 - 04.06.2018 =
* Fix vulnerable plugin issue
* Compatible with WordPress 4.9.6 and WooCommerce 3.4.2

= 1.0.9 - 22.05.2018 =
* .po file added

= 1.0.8 - 23.04.2018 =
* Check  WordPress and WooCommerce compatibility
* Normal Bug Fix.

= 1.0.7 - 31.01.2018 =
* Added Newslater Form.

= 1.0.6 - 23.10.2017 =
* FIX: Argument issue resolved for category search.

= 1.0.5 - 04.07.2017 =
* Check  WordPress and WooCommerce compatibility

= 1.0.4 - 13.04.2017 =
* Comptibility.
* Check Wordpress and WooCommerce compatibility.


= 1.0.3 - 04.04.2016 =
* Normal Bug Fix.

= 1.0.2 - 22.12.2016 =
* Compatible with WordPress Version 4.7 and WooCommerce 2.6.9

= 1.0.1 - 31.08.2016 =

* Check  WordPress and WooCommerce compatibility